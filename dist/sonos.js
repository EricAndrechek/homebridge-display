...
class sonos {
    ...
    refresh() {
        let headers = {
            Authorization:
                "Basic " +
                new Buffer(this.cid + ":" + this.secret).toString("base64"),
        ...
        request(options, (err, res, body) => {
            if (err) {
                ...
                this.log.debug(
                    "[SPOTIFY] - Access token generated: " +
    ...
    callback(code) {
        ...
        request(options, (err, res, body) => {
            if (err) {
                ...
                this.log.debug(
                    "[SPOTIFY] - Access token generated: " + this.access_token
                );
            }
        });
    }
            ...
            if (err) {
                ...
                try {
                    if (JSON.parse(body).error !== undefined) {
                        this.log.debug(
                            "[SPOTIFY] - " + JSON.parse(body).error.message
                        );
                    }
                } catch {}
            }
            ...
            get(access_token, log, port, "", function (result) {
                ...
                if (update_json.is_playing) {
                    try {
                        ...
                        update_json["device_type"] =
                            user_playback["device"]["type"];
                        update_json["device_id"] =
                            user_playback["device"]["id"];
                        update_json["volume"] =
                            user_playback["device"]["volume_percent"];
                        update_json["shuffle_state"] =
                            user_playback["shuffle_state"];
                        update_json["repeat_state"] =
                            user_playback["repeat_state"];
                    } catch (err) {
                        if (user_playback !== "API rate limit exceeded") {
                            try {
                                if (
                                    user_playback.currently_playing_type ===
                                    "episode"
                                ) {
                    ...
                    get(
                        ...
                        function (result) {
                            ...
                            try {
                                update_json["progress_ms"] =
                                    currently_playing["progress_ms"];
                                update_json["img_url"] =
                                    currently_playing["item"]["album"][
                                        "images"
                                    ][0]["url"];
                                let artist_names = [];
                                for (const artist of currently_playing["item"][
                                    "artists"
                                ]) {
