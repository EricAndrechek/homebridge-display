...
class HomebridgeDisplay {
        ...
        if (boxtype[i] === "spotify") {
          ...
          if (wet_settings !== false) {
            ...
            if (
              ...
              this.wet_obj = new wet(api_key, unit, lat, lon, this.log, this.config, this.api);
  ...
  createServer(
    ...
    server.listen(parseInt(this.config.Config.port), () => {
      log.debug("Starting Homebridge-Display server on port", parseInt(this.config.Config.port));
    });
  }
}
